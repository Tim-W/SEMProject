...
public class AreaTracker {
	...
	public void calculateNewArea(Point qixCoordinates, boolean fastArea) {
		...
		if (start.getX() != dir.getX() || start.getY() != dir.getY()) {
			// If stix was first moving in X direction get points above and
			// under the first stix point and start the floodfill algorithm from
			// there
			...
			if (start.getY() != dir.getY()) {
				// If stix was first moving in Y direction get points left and
				// right the first stix point and start the floodfill algorithm
				// from there
		...
		// Check in which of the two area the qix was found and set the other
		// one to the newly created area
		if (area1 == null) {
			newArea = area2;
			newBorder = border2;
		} else if (area2 == null) {
	...
	/**
	 * Floodfill algorithm, this algorithm checks one point if it's a point that
	 * gets added to new area and if the qix is found and adds that to one of
	 ...
	 * @param chosenState
	 *            begin state of the area that gets added to the new area,
	 *            practically always AreaStates.UNCOVERED
	 * @param addToArea1
	 *            boolean that keeps thrack of which temporary AreaTracker to
	 *            use.
	 */
		...
		try {
			if (boardGrid[(int) pointToCheck.getX()][(int) pointToCheck.getY()] == chosenState) {
				...
				if (pointToCheck.equals(qixCoorinates)) {
					// If that point was the coordinate of the qix set the
					// temporary area tracker that is currently in use to null
					if (addToArea1) {
						area1 = null;
						border1 = null;
					} else {
					...
					// If that point was not the coordinate of the qix add that
					// point to the right temporary area tracker
					if (addToArea1) {
					...
					// Check all the four neighbours of the current point
					// recursively
